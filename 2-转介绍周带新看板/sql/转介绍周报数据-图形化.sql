...
referral_week_data as (
  select
    ...
    sum(referral_data.reach) over (
      partition by referral_data.term_id,referral_data.package_grade
      order by
  ...
  from
    ...
    and referral_data.package_grade = user_attend_cnt.grade
...
ever_referral_data as (
  select
    ...
    referral_week_data.package_grade,
  ...
  from
    ...
    and referral_week_data.package_grade = user_base.package_grade
...
before_class_poster as(
  ...
  from
    ...
    and user_attend_cnt.grade = user_base.package_grade
    left join referral_data_tmp on user_base.user_id = referral_data_tmp.user_id
...
,
result_temp as(--无K值等计算字段的结果
 (select
    before_class_poster.term_id,
    before_class_poster.term_name,
    before_class_poster.package_grade,
  ...
  from
    ...
    and before_class_l1.package_grade = before_class_poster.package_grade
)
union
(
	select
    ...
    from
      ...
      and referral_week_data.package_grade = ever_referral_data.package_grade
      and referral_week_data.monday_of_week = ever_referral_data.monday_of_week
  ))

select *,
       --分年级计算--
       round(cum_succeed_l1/user_cnt,4) as cum_K,
       round(succeed_l1/user_cnt,4) as K,
       round(poster_cnt/user_cnt,4) as P,
       round(succeed_l1/poster_cnt,4) as R,
       round(browser_cnt/user_cnt,4) as `browser/user`,
       round(poster_cnt/browser_cnt,4) as `active/browser`,
       round(reach/poster_cnt,4) as radar,
       round(succeed_l1/reach,4) as CVR,
       round(ever_poster_cnt/user_cnt,4) as cum_P,
       round(if(datediff(current_date(),date(monday_of_week))>30,cum_succeed_l2/cum_succeed_l1,null),4) as renew_rate,
       round(ever_att_cnt/user_cnt,4) as cum_att_rate,
       round(att_cnt/user_cnt,4) as att_rate,
       round(ever_upload_cnt/user_cnt,4) as cum_upload_rate,
       round(upload_cnt/user_cnt,4) as upload_rate,

       --年级合并计算
       round(sum(cum_succeed_l1)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_cum_K,
       round(sum(succeed_l1)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_K,
       round(sum(poster_cnt)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_P,
       round(sum(succeed_l1)over(partition by term_id,monday_of_week)/sum(poster_cnt)over(partition by term_id,monday_of_week),4) as All_R,
       round(sum(browser_cnt)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as `All_browser/user`,
       round(sum(poster_cnt)over(partition by term_id,monday_of_week)/sum(browser_cnt)over(partition by term_id,monday_of_week),4) as `All_active/browser`,
       round(sum(reach)over(partition by term_id,monday_of_week)/sum(poster_cnt)over(partition by term_id,monday_of_week),4) as All_radar,
       round(sum(succeed_l1)over(partition by term_id,monday_of_week)/sum(reach)over(partition by term_id,monday_of_week),4) as All_CVR,
       round(sum(ever_poster_cnt)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_cum_P,
       round(if(datediff(current_date(),date(monday_of_week))>30,
           sum(cum_succeed_l2)over(partition by term_id,monday_of_week)/sum(cum_succeed_l1)over(partition by term_id,monday_of_week),
           null)
           ,4)as All_renew_rate,
       round(sum(ever_att_cnt)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_cum_att_rate,
       round(sum(att_cnt)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_att_rate,
       round(sum(ever_upload_cnt)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_cum_upload_rate,
       round(sum(upload_cnt)over(partition by term_id,monday_of_week)/sum(user_cnt)over(partition by term_id,monday_of_week),4) as All_upload_rate
