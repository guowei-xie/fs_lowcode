...
records = records.drop('record_id', axis=1)
#
# # records['字段 1'] = records['字段 1'].astype(int)
# # records['字段 1'] = records['字段 1'].astype(int)
#
#
bitable.refresh_records(table_id=table_id, df=records) # 将指定table_id的数据清空并将DataFrame数据批量插入

# # bitable.insert_records_from_df(table_id=table_id, df=df) # 在指定table_id末尾插入DataFrame
# #
# # bitable.insert_records_from_df(table_id=table_id, df=records)
